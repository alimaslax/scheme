...
(define (make-new-machine)
    (let ((pc (make-register))
          ...
          (labels '())
          (breakpoints '()))   ;; list of pairs (labels & offsets)
      (let ((the-ops
        ...
        (define (execute)
          (let ((insts (get-contents pc)))
            (if (null? insts)
                'done
                (begin
                  ;; check for a breakpoint before actually executing. only the
                  ;; breakpoint's counter matters here, so a custom filter is
                  ;; used instead of remove-val.
                  (let ((breakpoint-at-this-point
                         (filter
                          breakpoints
                          (lambda (breakpoint)
                            (= (breakpoint-counter breakpoint)
                               (instruction-line-counter (car insts)))))))
                    (if (null? breakpoint-at-this-point)
                        (actually-execute) ;; go on as usual
                        ;; else, take what
                        ;; should be only one breakpoint and print out its
                        ;; details.
                        (let ((breakpoint (car breakpoint-at-this-point)))
                          (display* "Breakpoint: "
        ...
        (define (dispatch message)
          (cond ((eq? message 'start)
                ...
                ((eq? message 'install-labels)
                 (lambda (labs) (set! labels labs)))
                
                ((eq? message 'add-breakpoint)
                 (lambda (label n)
                   (set! breakpoints (cons (create-breakpoint label n labels)
                                           breakpoints))))
                ((eq? message 'remove-breakpoint)
                 (lambda (label n)
                   (let ((bp (create-breakpoint label n labels)))
                     (if (member bp breakpoints)
                         (set! breakpoints (remove-val bp breakpoints))))))
                 ((eq? message 'remove-all-breakpoints)
                  (lambda () (set! breakpoints '())))
                 ((eq? message 'continue)
                  (lambda () (actually-execute)))  ;; restart iteration.
                (else (error "Unknown request -- MACHINE " message))))
        dispatch)))

;; the label and offset when creating a breakpoint are needed for printing out
;; breakpoint details and determining if a breakpoint needs to be removed from
;; the breakpoint list. the true important value when executing the program is
;; its counter, the counter of the label plus the offset n.

(define (breakpoint-label breakpoint) (car breakpoint))
(define (breakpoint-n breakpoint) (cadr breakpoint))
(define (breakpoint-counter breakpoint) (caddr breakpoint))

(define (create-breakpoint label n labels)
  (let ((label-entry (assoc label labels)))
    (if label-entry
        (list label n (+ n (label-entry-counter label-entry)))
        (error "Label not found -- CREATE-BREAKPOINT" label))))

;;; from hw2: filter, remove-val
...
(define (assemble controller-text machine)
  (extract-labels controller-text
                  (lambda (insts labels)
                    ...
                    ((machine 'install-labels) labels)  ;; save labels into
                                                        ;; machine
                    insts)))

;;; change for breakpoints: some type of order of instructions must be saved to
;;; compare to pc register, the program counter.

(define (extract-labels text receive)
  (define (extract-labels-with-counter text receive counter)
    (if (null? text)
        (receive '() '()) ;; This is where everything really happens, at the
                          ;; end.
        (extract-labels-with-counter
         (cdr text)
         (lambda (insts labels)             ;; This is where labels and insts
                                            ;; are
           (let ((next-inst (car text)))    ;;   accumulated (as on a stack):
             (if (symbol? next-inst)
                 (if (assoc next-inst labels)
                     ...
                     (receive
                      insts                     ;; either like this
                      (cons (make-label-entry next-inst  ;;  (it's a label)
                                              insts
                                              counter)
                            labels)))
                 (receive (cons
                           (make-instruction next-inst counter) ;; or like this
                           insts)                       ;;  (it's an inst)
                          labels))))
         (if (symbol? (car text))
             counter
             (+ 1 counter)))))   ;; count only instructions in the program
                                 ;; counter. Labels are removed on assembly.
  (extract-labels-with-counter text receive 0))
...
;;; make-instruction creates a list whose first element is the instruction text
;;; and whose second element is initially empty.  The second element will later
;;; be filled in by update-insts! using set-instruction-execution-proc! The
;;; third element is its counter relative to the whole program.

;;; Addition for breakpoints: counter.
...
(define (instruction-line-counter inst) (caddr inst))

(define (set-instruction-execution-proc! inst proc)
  (set-car! (cdr inst) proc))  ;; can't do (set! (cadr inst), this is clearer
                               ;; anyways

;;; make-label-entry creates a list whose first element is the label and whose
;;; second element is the "rest of the instruction sequence".  Thus, branching
;;; to that label amounts to resuming execution at the second element of the
;;; pair.  (One could think of the second element of the pair as the address in
;;; instruction memory referred to by the label.) The third element is its
;;; counter relative to the whole program.

;;; Addition for breakpoints: counter.

(define (make-label-entry label-name insts counter)
  (list label-name insts counter))

(define (label-entry-name entry) (car entry))
(define (label-entry-insts entry) (cadr entry))
(define (label-entry-counter entry) (caddr entry))

(define (lookup-label labels label-name)
  (let ((val (assoc label-name labels)))
    (if val
        (label-entry-insts val)
