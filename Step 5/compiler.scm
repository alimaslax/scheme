...
(define (compile exp target linkage)
  (cond ((self-evaluating? exp)
        ...
        ((or? exp) (compile (or->if (or-exps exp)) target linkage))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;;     basic apply - similar to compile-application
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(define (compile-apply exp target linkage)
  (let
      ((proc-code (compile (apply-proc exp) 'proc 'next))
       (operand-codes (compile (apply-params exp) 'argl 'next)))
    (preserving '(env continue)
                proc-code
...
(define (compile-map exp target linkage)
  (let
      ((proc-code (compile (apply-proc exp) 'proc 'next))
       (operand-codes (compile (apply-params exp) 'temp1 'next)))
    (preserving '(env continue)
                proc-code
                (preserving '(proc continue temp1 temp2)
                            (append-instruction-sequences
                             operand-codes
                             (temp1-loop) ;; clearer to separate this out
                             (make-instruction-sequence
                              '(argl) (list target) (list target)
                              `((assign ,target (reg temp2)))))
                            (compile-linkage linkage))))
)

(define (temp1-loop)
  (let ((done-label (make-label 'temp1-done))
        (continue-label (make-label 'temp1-loop)))
    (append-instruction-sequences
     (make-instruction-sequence
      '(argl temp1 temp2)
      '(argl temp1 temp2)
      ...
      `(
        ...
        (assign argl (op car) (reg temp1)) ; only one arg for basic map
        (assign argl (op list) (reg argl)) ; but still has to be a list
        (save temp1) ; used as storage of proc call below
        (save proc))) ; the proc can change on a user-defined procedure
     (compile-procedure-call 'temp1 'next)
     (make-instruction-sequence
      '(temp1 temp2)
      '(temp1 temp2)
      '(temp1 temp2)
